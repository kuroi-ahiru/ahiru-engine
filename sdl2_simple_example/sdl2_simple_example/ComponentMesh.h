...
public:
    ...
    ComponentMesh(GameObject* owner, const std::vector<glm::vec3>& vertices,
        ...
        : Component(Type::Mesh, owner), vertices(vertices), texCoords(texCoords), indices(indices),
        textureIDs(textureIDs), showNormals(false), visible(true) {}
    ...
    void Render() override {
        ...
        if (showNormals) {
            RenderNormals();
            glColor3f(1.0f, 1.0f, 1.0f); // Restaurar color tras dibujar normales
        }
    }
...
private:
    ...
    /*GLuint textureID;*/
    std::vector<GLuint> textureIDs; // Almacena las texturas asociadas
    bool showNormals;
    bool visible;
