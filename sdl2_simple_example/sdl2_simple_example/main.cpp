...
//        projectionMatrix = glm::perspective(glm::radians(fov), (float)WINDOW_SIZE.x / (float)WINDOW_SIZE.y, 0.1f, 100.0f);
//    }
...
int main(int argc, char** argv) {
    ...
    // Crear cámara
    std::shared_ptr<Camera> camera = std::make_shared<Camera>(45.0f, (float)WINDOW_SIZE.x / (float)WINDOW_SIZE.y, 0.1f, 100.0f);

    // Crear renderizador y pasar la cámara
    Render renderer(WINDOW_SIZE.x, WINDOW_SIZE.y, camera);
    ...
    while (window.processEvents() && window.isOpen()) {
        ...
        if (!window.IsPaused()) {
            ...
            camera->processKeyboardInput(SDL_GetKeyboardState(NULL), FRAME_DT.count());

            // Procesar la entrada del ratón
            int mouseX, mouseY;
            Uint32 mouseState = SDL_GetMouseState(&mouseX, &mouseY);
            rightButtonPressed = (mouseState & SDL_BUTTON(SDL_BUTTON_RIGHT)) != 0;
            camera->ProcessMouseInput(mouseX, mouseY, rightButtonPressed, FRAME_DT.count());
